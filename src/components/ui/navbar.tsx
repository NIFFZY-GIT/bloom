...
export default function Navbar({ isAuthenticated, userRole }: NavbarProps) {
  ...
  const isBookingsActive = navItems.find(item => item.id === 'bookings')?.dropdown?.some(d => d.id === activeLink) || false;

  const getIsActive = (item: any) => {
    return activeLink === item.id || (item.dropdown && item.id === 'bookings' && isBookingsActive);
  };
    ...
    <nav className={`navbar ${isScrolled ? 'scrolled' : ''}`}>
      <div className="nav-container">
        ...
        <div className={`nav-links ${isMenuOpen ? 'nav-links-active' : ''}`}>
          {navItems.map((item) => (
            <div 
              ...
              onMouseEnter={() => !isMobile && item.dropdown && handleBookingsHover(true)}
              onMouseLeave={() => !isMobile && item.dropdown && handleBookingsHover(false)}
            >
              {item.dropdown ? (
                <button
                  type="button"
                  className={`nav-link ${getIsActive(item) ? 'active' : ''} dropdown-toggle`}
                  onClick={(e) => {
                    e.preventDefault();
                    if (isMobile) {
                      setIsBookingsDropdownOpen(prev => !prev);
                    }
                  }}
                >
          ...
          {/* Mobile Auth Section */}
          {!isAuthenticated ? (
            <button className="auth-button mobile-auth" onClick={handleAuthClick}>
              <span>Login</span>
              <div className="cta-icon">👤</div>
            </button>
          ) : (
            <div className="mobile-auth-container">
              <button className="auth-button mobile-auth-trigger" onClick={handleAuthClick}>
                <span>{userRole === 'ADMIN' ? 'Admin' : 'Profile'}</span>
        ...
        <div className="desktop-cta-group">
          <button className="book-now-button desktop-cta" onClick={() => router.push('/bookings')}>
            <span>Book Now</span>
            <div className="cta-icon">✈️</div>
          </button>

          {/* Desktop Auth Dropdown */}
          <div className="auth-dropdown-container ml-3">
            <button
      ...
      <style jsx>{`
        ...
        /* Navigation Links - No Text Decoration */
        .nav-link,
        .nav-link:link,
        .nav-link:visited,
        .nav-link:active,
        ...
        /* Dropdown links - No Text Decoration */
        .dropdown-link,
        .dropdown-link:link,
