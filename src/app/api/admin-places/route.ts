...
export async function GET() {
  try {
    ...
    console.log('[API] Admin Places fetched:', result.rows.length);
    console.log('[API] Admin Place data:', result.rows.map(p => ({ name: p.name, image: p.image, categoryId: p.categoryId })));
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log('[API] Admin Place created:', newPlace);
    ...
    console.error('Failed to create admin place:', error);
